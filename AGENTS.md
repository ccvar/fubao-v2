...
中心库只同步仍在有效期内的红包。已明确过期的红包不得进入上传队列、不得由服务端接纳、也不得下发到其他客户端；客户端启动恢复旧同步队列时必须自动剔除其中已经过期的红包，并让有效红包优先于批量直播间快照上传，避免短有效期红包被历史回灌阻塞。红包一旦从中心库获取过，其中心来源标记必须持久保留；后续本地监测补齐原生参与参数只能改变数据所有权，不得抹掉前端可见的中心来源。

中心库红包增量使用独立的两秒原生拉取节奏，不得被十秒一次的直播间/全量上传快照阻塞；收到中心增量后，前端仍通过安全的 Go 列表接口刷新，不暴露中心设备令牌或任何账号凭证。

参与记录中，“未中奖”只作为结果徽标；其下方流程状态显示“已开奖”，避免重复显示“未中奖”。

参与记录结果徽标的暗色 Tooltip 必须挂载到页面顶层，不能被参与记录表格的滚动容器、粘性表头或 `overflow` 裁切。

红包参与接口 `luckybox/join` 返回 `status_code=0` 且 `succeed=false`（裸 soft-deny，无 rush_spam/操作频繁等明确风控文案）时记为**参与失败**：参与记录显示“参与失败”，**不**进入风控冷却、不结束账号参与任务、不阻止后续分配。仅当响应含明确频率/风控标记（如 `rush_spam`、操作频繁）时才记为“风控冷却”，账号进入可配置冷却（默认 60 分钟，设置-红包参与），期间不接收新任务；单独启动提示“该账号冷却中”，批量启动跳过冷却中账号。账号一旦因明确风控进入冷却且无待开奖记录，立即结束该账号当前参与任务并释放原生页面上下文；批次内全部账号结束后批次摘要一并收尾。风控冷却的 `red_packet_cooldown_until` 与“风控冷却”状态文案在任务停止、关闭红包接口开关、以及后续 cooldown=0 的参与记录写入后仍必须保留，直到冷却到期；实例卡片与参与账号行的冷却倒计时徽标同理。点击实例红包图标准备页面上下文时，须在 live 导航后充分等待登录就绪（含 jar Cookie 可读），不得因 SPA 短暂 Unknown 误报“直播页面尚未完成加载”。成功参与在 UI 上显示 **2 次请求**（`join` 受理 + `receive` 开奖）；仅 1 次且接口=join 表示 join 未受理。页面 join 必须纯接口（无 DOM 点击兜底），查询参数对齐真实 live 页捕获（`enter_from=link_share`、`enter_from_merge=link_share`，不强制附加 `web_rid`/`box_type`）。
...
实例记录是可丢弃的:关闭卡片再为同一账号创建会得到新的 instance id,但原生数据存储仍按账号 id 复用。因此嵌入式注入在 macOS 也必须先判断残留会话——读取 jar,只有当它非空且无法证明当前 store 登录态时才清理 Douyin 行,能证明时绝不清理(无条件清理会抹掉有效的扫码会话)。落地页记忆同时按 instance 与账号保存,删除重建的卡片恢复账号上次那个轻量页面而不是回退到最重的推荐信息流;首次加载已经渲染出已登录文档时跳过第二次整页重载,那次重载是挂载耗时的主要来源。

抖音"匿名外壳"(页面顶栏出现可见的"登录"入口)与登录墙、CK 失效都是不同状态:它表示文档在注入的 Cookie 生效前就渲染完成,是过期的渲染而不是凭据问题。登录探针必须把它作为独立信号返回,不参与 CK 判定。挂载序列遇到它要走与登录墙相同的"清 SPA 存储 + 重新注入 + 重载"恢复;后台自适应轮询遇到它按每 3 分钟一次限流自愈,期间不得声称登录正常、也不得据此立刻判失效;只有持续 8 分钟以上、jar 里始终握着 store 登录 Cookie 却依然渲染匿名外壳,才通过原生通道上报一次未登录以便卡片给出"重新绑定 CK"。匿名判定必须是三态的:顶栏在屏且没有登录入口才算"干净",页面尚在水合或顶栏滚出视野时只能"不表态",绝不能把"还没渲染出登录按钮"或"登录按钮被滚出可视区"当成已登录。抖音的登录入口是带混淆类名的 `div`、没有 `data-e2e` 钩子,所以扫描必须覆盖 `div/span` 并识别稳定 id `#login-panel-new`;"有登录入口"与"顶栏就绪"两侧的可见性判断必须对称,否则滚动就能把匿名页读成已登录。登录探针**不得用 `innerText` 判断页面是否已渲染**：`innerText` 依赖布局，被隐藏或尺寸为 0 的子 WebView 即使文档完整也一律返回空串，于是任何曾被隐藏过的卡片都会永久落进"不表态"，表现为卡片停在抖音骨架屏、`login-state` 永远 unknown。就绪判断用与布局无关的 `textContent` 长度；登录墙文案匹配仍留在 `innerText` 上，因为"没渲染出来"必须保持无结论而不能变成 CK 失效。三态探针的"不表态"必须自己有出路：抖音的骨架屏可以无限期停在既不干净也不匿名的中间态，那样匿名自愈与登录墙判定都不会介入，卡片会永远停在灰色骨架上。连续 90 秒都拿不到结论时按每 180 秒一次限流做一次恢复（重新注入 + 清 SPA 存储 + 重载），并且绝不由这个信号推导任何 CK 结论。已准备好红包参与上下文的实例完全不参与匿名判定与自愈:直播间导航后的签名请求模板就活在那个文档里,清 SPA 存储并重载会把正在运行的任务赖以生存的上下文直接销毁,表现为随后的「浏览器实例未进入目标直播间」。参与期间出现的匿名帧交由参与路径自己处理,后台轮询既不启动匿名计时也不重载页面。脱离匿名状态需要干净证据并持续满 2 分钟安静期,不表态的帧不改动任何计时;只要匿名计时仍然挂着,就绝不把该实例同步成 CK 正常。这个信号只能用来触发有限次自愈重载,**不得用来判定 CK 失效**:卡片页面窄小、可滚动、还会间歇弹出抖音自己的登录面板,DOM 启发式无法区分"会话被拒"和"顶栏尚未绘制",实测会让账号在失效与正常之间来回翻转。可以判定 CK 失效的权威证据只有两种:认证过的业务请求(例如关注直播接口返回 `20003` 而非 `0`),以及**持续存在的显式登录墙**(整块"登录后免费畅享高清视频 + 扫码登录 + 验证码登录"对话框)。登录墙是文案匹配而不是几何启发式,抖音不会对已登录用户弹它,所以它必须走到判定逻辑而不能被匿名自愈分支吞掉。登录墙的宽容期必须有时间上限:一次墙出现时只允许重新注入一次 store Cookie(那一次才刷新注入宽限期),此后不得再刷新宽限期;墙连续存在满 90 秒即上报未登录,让卡片给出"重新绑定 CK"。旧实现只要 store 与 jar 都还握着登录 Cookie 就无限期推迟判定,而每次推迟都重新注入并刷新 30 秒宽限期,下一轮检查又提前返回,形成永远不会失效的闭环。结束一次登录墙计时需要"已登录且干净渲染"的正面证据,重载中途的不表态帧不得清零计时。关注直播页面请求的结论必须真正写回账号健康:`20003` 立即上报未登录,`0` 立即上报已登录并清零登录墙计时,与已持久化状态相同时不重复上报。反向翻转同样要防:**已失效账号不得靠 DOM 页面观察被提升回正常**,因为客户端重启时文档会在抖音弹出登录对话框之前就渲染完,那一帧看起来像已登录,曾把接口仍在返回 `20003` 的会话重新标成正常;清除失效徽标只能依靠关注直播接口这类认证业务请求,或用户显式重新绑定 CK。用户显式触发的操作保持原有判定,不被这条自愈逻辑阻断。
...
While a consolidated participation batch is active, the compact top-bar runtime summary exposes an icon-only stop action at its far right. Stopping always requires the shared compact confirmation dialog, stops the whole batch, prevents future allocations, and does not forcibly interrupt a single request that has already been sent. The recent-activity batch stop action uses the same confirmation path.

In “任务运行记录”, each collapsed date summary shows task count, the distinct participating-account count for that calendar day (deduplicated by canonical account ID across all runs), accepted participation count, win count, and confirmed diamonds in that order: `N 个任务 · N 个账号 · N 次参与 · N 次中奖 / N 钻`. Repeated runs by the same account must never inflate the daily account count.
...
On the browser-instance title subtitle, after CPU/memory usage show a compact text control “清空风控冷却” that clears every participation account’s risk-control cooldown timer, clears the risk badge status, and re-enables the red-packet API switch for those accounts (challenge blocks are left alone).
...
The data-management screen is named “账号与红包池” and orders its primary tabs as “参与账号 / 红包池 / 参与记录”, with “直播间 / 监测账号” still collapsed behind the compact 监测管理 control. The red-packet event tab label is “红包池” (not bare “红包”). The red-packet monitor reuses 福宝’s signed `luckybox/box/list` request path (with `lottery_info` as a safe fallback) but must surface only explicit 红包 payloads; 福袋/lottery payloads are filtered out. Legacy room migration copies `rooms_config.json` into the Go engine’s permission-restricted local store and must never modify the old 福宝 data.

每一处原子写入都必须使用**写入者唯一的临时文件名**(`os.CreateTemp(dir, base+".tmp-")`),绝不能用固定的 `<文件名>.tmp` 兄弟文件。`DefaultDataDir()` 只按 bundle identifier 定位,dev 构建与已安装的正式版解析到**完全相同的数据目录**,于是两个引擎进程会写同一个临时文件:字节流交错之后 rename 把损坏内容发布成正式文件——`accounts.json` 里存着全部 Cookie,一次撕裂写就是全部凭据。失败路径必须删掉自己的临时文件(用 `defer`,rename 成功后它自然是空操作),不留残留。恢复逻辑要同时认旧的固定 `.tmp` 名(老版本写的)和新的 `.tmp-*` 通配。

同时注意:**两个引擎共用一个数据目录本身就是有害的**,即使写入不再损坏也一样。两边各自在内存里持有整个 store 并定期全量序列化回写,后写的一方会用自己那份(不含对方改动)覆盖掉先写的一方;两边还会各自按整机资源算出同一个浏览器并发上限并各自admit满,以及各自按账号数推导出同一个全局请求节奏,使实际发向抖音的速率翻倍。开发时请用 `FUBAO_DATA_DIR` 指向独立目录。 The top-bar action is named “导入数据”, with “导入直播间” kept as a distinct first menu action.
...
Room red-packet monitoring is explicitly two-stage. First probe Douyin's room-entry endpoint to determine whether the room is live; only a room positively confirmed live may query red-packet endpoints. Unknown rooms use a short initial probe cadence, offline rooms use a slower live-status cadence, live rooms use a faster red-packet cadence, and an active red packet uses the fastest cadence. 未能得出明确开播/未开播结论的房间必须走**退避阶梯**而不是永远停在最短的 unknown 节奏：每次连续的 unknown 或请求错误把该房间自己的间隔翻倍（unknown 上限 10 分钟，错误上限 5 分钟），任何一次明确的开播/未开播结论立刻清零。大批量导入里失效、已删除、填错的房间号全部只会回答 unknown，按固定短节奏它们消耗的预算是正常离线房间的数倍，足以吃光整机请求预算并饿死真正在直播的房间。账号池冷却不是房间的问题，不得计入该房间的阶梯。退避只影响该房间的探测频率，永远不构成"未开播"证据，因此不能影响自动回收。 Monitor-task state, live/offline state, and red-packet state are separate UI concepts. In the room table, combine the streamer and room identifier into one column with the identifier and source shown below the streamer, and show the current live/offline result in the status area. Place an icon-only funnel control beside the “主播 / 房间标识” heading and reuse the compact room-sort menu for the Go-backed source filter “全部来源 / 关注列表 / 导入 / 中心库”; only a permanent professional license may see the “中心库” option.
...
来源顺序之上还有一层**开播档位**：确认正在直播的房间排在所有非直播房间之前，来源顺序只在同一档位内部生效。只按到期时间排序在房间数超过请求预算时会失效——几千个已经逾期几分钟的离线探测，其到期时间戳永远早于一个 15 秒后才到期的直播间，于是 5 秒/15 秒/60 秒的节奏分级会退化成一条统一的慢轮转，直播间与死房间同等对待，红包窗口必然错过。只有明确探测为直播才进快档，unknown 与 error 一律留在慢档，避免大批量死房间稀释快档。快档同样有 burst 上限，保证慢档持续被探测、新开播的房间仍能被发现。

The shared settings dialog includes a third top-level “监测设置” tab. Its Go-persisted, hot-applied controls are the global request interval, per-account request interval, global slow-request concurrency, per-account slow-request concurrency, and native probe concurrency. Saving affects subsequent requests in an already-running monitor pool without cancelling in-flight requests; unsafe values are clamped in Go. Keep the internal bulk-worker sharding fixed and unavailable as a user-facing tuning control——批量监测的工作协程数量直接跟随"原生探测窗口"，不另加控件；旧实现把它写死成 64，使"全局慢请求并发"和"原生探测窗口"调到 64 以上完全无效，且改设置对正在运行的池毫无作用。改设置必须热调整正在运行的工作协程数量，不需要全部停止再全部启动。

全局请求间隔的 **0 表示"自动"，并且是默认值**：由 Go 引擎按可用监测账号数推导（约 `单账号请求间隔 ÷ 账号数`，下限 5 毫秒，且不会比历史固定值 80 毫秒更慢），账号池成员变化时热重算。固定的全局节奏会让整机吞吐与账号数量脱钩——80 毫秒把整机锁死在约 12.5 请求/秒，无论导入多少监测账号；102 个账号本可支撑约 136 请求/秒，九成容量被浪费，6000+ 房间的实际轮转会退化到接近 9 分钟。显式填写的正数一律尊重，不被自动推导覆盖，并且版本迁移只把仍停在旧固定默认值的存档改为自动。全局节奏与单账号节奏都必须在**占用并发槽位之前**完成等待：把限速等待算进并发槽位会让"并发"不再表示真正在途的请求数，也会让 goroutine 攥着账号槽位空等全局队列，把整个账号池闲置。

监测请求必须共用一个连接池：`accountPool` 持有一个共享的 HTTP 客户端，每轮探测只新建轻量的 `httpclient.Client`（只有 Referer 是按房间变化的）。为每次探测新建 `http.Transport` 意味着每个房间每一轮都要重做 DNS、TCP 与完整 TLS 握手，几千个房间时这项开销主导整轮耗时，还会制造大量无法复用的空闲连接与 socket 抖动，反过来触发连接错误并把账号误推进冷却。共享 transport 的 `MaxIdleConnsPerHost` 必须显式调大（Go 默认只有 2），并且要随「全局慢请求并发」热增长：否则在运行中调高并发,超出旧连接池大小的那部分槽位又会退回每请求一次 TLS 握手。只增不减,降低并发时保留已经热起来的连接。

账号池整体冷却时不得让每个房间按固定 1 秒重试：必须使用 `poolUnavailableError` 自带的 retryAfter 提示（上限 30 秒）。几千个房间每秒空转一遍调度器只会产生纯 CPU 与锁竞争，没有任何网络产出。

房间探测状态是可再生数据，落盘窗口按 monitor 数量自适应放宽（约每 2000 个房间多 3 秒，上限 30 秒），避免一整场监测期间每 3 秒把上万条记录重新序列化一次。红包参与的预定与结果不走这条合并路径，仍然同步落盘。
...
监测账号池的退避窗口必须**持久化并在账号行上可见**,否则被限流的账号会一直显示「可用」却已经悄悄停止发请求——这是提高整机速率之后最需要看见的信号。`markFailure` 设置冷却时通过安全 hook 把 `账号 id + 到期时间 + 原因` 写进账号 store(不含 Cookie、请求参数或响应),`markSuccess` 清除。原因至少区分**限流**(444/429)、**鉴权**(401/403)与**网络**(连续超时/传输失败),因为处理方式完全不同:限流说明本机整体太快,鉴权指向这一个会话。徽标文案按原因区分为「限流冷却 / 鉴权冷却 / 网络冷却」并带倒计时,但 `accountStatus` 仍然只返回「可用 / CK 失效 / 冷却中」三个值,以保持状态筛选器的既有契约。这条链路**绝不允许改写 monitoring 的 CK 健康**:被拒绝的请求不是会话已死的证据,上面那条规则依然成立,悬浮说明里要明确写出「CK 仍然有效,无需重新绑定」。退避窗口属于创建它的账号池,不能比池活得更久:引擎启动时与「全部停止」时都必须清空,否则界面上会出现无人执行的倒计时。
...
这条链路的**累积性**故障(切换几次才坏)有三个必须同时守住的规则。其一,几何同步的 revision 必须在**排队任务真正开始执行时**读取,而不是在调度时捕获:同步任务会在共享的原生队列里排队,期间发生一次切页就让它到达时自我作废,而且没有任何东西会重新调度它——切换次数越多越容易撞上,这正是"多切几次就坏"的来源。任务执行时才测量几何,所以用当前 revision 校验才是对的;revision 仍然防护同步**过程中**的变更。其二之补充:把租约刷新移出原生队列**同时也移走了它唯一的背压**。原来 acquire 是 `await` 在串行队列里的,天然一次只有一个;改成 fire-and-forget 之后必须自己限流,否则每次几何同步都能并发甩出 N 个 acquire,全部涌向引擎那条严格串行的 RPC 循环。限流要点有三:节流时间戳在**发起前**就写入;失败时**不得清零**节流(引擎越慢越不能重试风暴);以及尚未持有租约的实例**也要有节流下限**——"没有租约就算 stale"会让节流完全失效,实测会把 mount 自己的 acquire 饿死在队列里,表现为标题栏 `0 个运行`、卡片永远停在"正在加载实例"。每个实例还要有 in-flight 保护,同一实例不得有两个 acquire 在飞。

其二,运行租约刷新**绝不能在原生布局队列里被 await**:所有原生几何操作共用一条串行队列,而引擎 RPC 又是串行单循环,监测繁忙时一次租约往返是秒级,会把后面所有几何工作全部堵死,连锁引发上面那种作废。租约刷新必须 fire-and-forget 并自带节流(失败时清掉节流以便下一拍重试)。其三,scroll / resize / IntersectionObserver / 响应式更新都会调度同步,必须**合并**成至多一个待执行任务,否则它们进入队列的速度快过排空速度;一个待执行任务永远足够,因为它在执行时才测量几何。
...
关注直播页面探测与红包参与共用同一个账号文档，所以自动/周期性的关注直播刷新必须跳过持有红包参与上下文的实例，原生命令侧同样直接拒绝：房间发现随时可以重试，抢包窗口不能。
...
Browser-instance refinements: the identity-row instance tile is 18px, and the whole footer row stays around 26px high with restrained vertical padding and 20px action controls. Account names and Douyin IDs expose the shared dark in-app Tooltip only when the rendered text is actually truncated. Keep the red-packet participation control inline, but consolidate “Chrome 修复登录”, “关闭实例”, and “打开实例” behind one compact three-dots action trigger. Every dropdown item displays its original operation icon followed by the operation name; unavailable Chrome repair remains visibly disabled for non-import accounts. Keep this menu notably denser than a dialog (compact rows, restrained padding and shadow), and suppress the trigger Tooltip for the entire time the menu is open so it never overlaps the actions. Closing always requires the compact confirmation dialog, then destroys the mounted child WebView and removes the card while preserving the account-keyed profile for later reuse. Chrome repair only launches the rescue CDP path and never switches the default card host away from embedded WebView. The CK-expired badge keeps its coral surface unchanged on hover and does not add a gray hover fill. When participation CK is expired, hide the red-packet participation icon in both the instance card and participation-account row instead of leaving an unusable control visible. The “新建实例” dialog supports multi-select batch creation for eligible participation accounts, while accounts that already own an instance remain excluded.
...
批量/轮换启动单个账号时等待页面就绪的上限可配置,但**默认必须与手动路径一致(18 秒),调低有真实风险**。`wait_for_douyin_login_ready` 超时后返回的是最后一次快照而**不是错误**,所以缩短它并不会跳过这个账号,而是拿一个尚未水合完成的文档去注册参与上下文——此时 `bdms.js` 还没装好重签 `msToken`/`a_bogus` 的 fetch 钩子,join 就会以未签名的形式发出,被抖音直接软拒。实测把默认值从 18 秒降到 10 秒后,`join_diag.log` 里的 join **从 100% 已签名(`msToken=1 a_bogus=1`)变成 100% 未签名,成功率归零**。要用"换下一个账号"换取吞吐,前提是先让"就绪"真正表示 bdms.js 已生效,而不是"deadline 到了"。

同理,参与任务结束后**不得导航正在运行的直播间文档**,只能改写落地页记忆。重签用的 `bdms.js` 钩子活在 `live.douyin.com` 那个文档里,导航走会销毁它,而缓存的请求模板和已注册的上下文都还在——这个窗口里派发的 join 同样会未签名发出并被软拒。卡片随后本来就会被关闭或复用,下一次挂载自然落在轻量首页,这才是原本要的效果。
...
**部分红包自带参与门槛**（送人气票需 N 钻、需粉丝团灯牌），普通 join 只会换来裸 soft-deny，必须识别并跳过，且这不是可配置项。`luckybox/box/list` 从不把门槛归一化成单个字段，所以沿用 DY-KIRO `red_packet_conditions.py` 的推断：递归扫描该活动分组的每个标量叶子，用 `(?:需|需要|消耗|支付|扣除)\s*(\d+)\s*个?钻` 与 `(?:需|需要|要求|必须|请先|先|点亮|加入|开通)\s*(?:粉丝团)?灯牌|粉丝团点亮|加入粉丝团` 匹配文本（`meta.before_unpack_desc` 是最常见来源），另外按键名读取 `need_diamond*`/`diamond_cost*`/`consume_diamond*` 的裸数字与 `popularity_ticket_gift.diamond_count`，归一化成紧凑文案如 `需1钻、需灯牌`。门槛存在 Go 事件的 `condition` 字段并同步中心库（服务端列 `participation_condition`），红包池在奖品后显示紧凑徽标并用共享暗色 Tooltip 说明不会参与；调度器在派发前直接跳过，不产生参与记录也不计入任务次数。列表 payload 沉默时，被拒响应的可读文案（例如 `请先加入粉丝团`）才暴露门槛：只解析该文案（不扫原始 JSON 键，`gift_name` 一类会误报），记为参与失败并回写事件门槛，让后续账号不再重复撞同一红包；这条失败不进入风控冷却。
...
倒计时窗口越窄，受理率越低：账号锁、有界 worker 槽位、钱包基线、anchor 补齐都排在派发决定与请求之间，原生侧还要导航直播间并确认登录，这些耗时全部从窗口里扣。实测同机同段时间内 30 秒快照的 join 全部被受理，10 秒快照全部裸 soft-deny，其中若干次甚至发在过期之后。因此**必须在真正发送前复校截止时间**：Go 在 `attempt` 之前重读 `eventDeadline`，原生侧在导航前与登录就绪之后各校一次（截止时间由任务自带的 `send_time + delay_time` 推出，缺字段时视为未知而不是已过期）。已过期的 join 一律不发，走无请求路径释放持久化占位，不产生参与记录也不计入任务次数；`receive` 不受此限制，个人开奖结果本来只存在于截止之后。设置界面在倒计时小于 20 秒时给出明确提醒，但不强制夹取用户的取值。

参与账号的钻石余额由 Go 原生层通过钱包接口读取并持久化为安全快照，账号列表只显示钻石数和最近同步状态，不得暴露 CK 或钱包原始响应。参与前记录余额基线；开奖接口缺失、空结果或无法关联个人结果时，只有钱包余额出现明确正增量才可兜底确认中奖并记录增量钻石，否则保持未中奖或待确认，不能把钱包请求失败直接标记为开奖异常或 CK 失效。裸 soft-deny（status=failed、succeed=false）同样执行钱包增量兜底：若钻石余额相对参与前基线有明确正增量，参与记录保持“参与失败”并在文案后附加“已中N钻（钱包增量确认）”，计入中奖统计；钱包请求失败时不改失败结论。

开奖结果查询设置使用“开奖后查询延迟”（默认 1 秒）和“最多查询次数”（默认 3 次）；延迟从该账号的参与请求被受理后计算，不得等待事件的过期/开奖时间戳。查询次数耗尽后必须立即执行钻石增量兜底，不能再以红包有效期或一个独立的“开奖异常等待”时长拖延参与记录。旧版开奖等待字段只用于兼容迁移，不再出现在设置界面。「页面上下文未就绪」不是一次查询尝试：卡片重挂载只需几秒就能恢复，所以要在有界等待预算（默认 45 秒）内轮询等它回来，不得把尝试次数消耗在一次都没发出的请求上。等待预算耗尽且从未发出 receive 时，记录文案必须是“直播页面上下文中断，未发出开奖请求”，不能说成“接口未返回”。单次开奖请求超时按开奖自己的预算（20 秒）计，不沿用 join 的 45 秒房间导航预算——一次三轮 45 秒会把账号唯一的未开奖名额堵住两分钟以上。钱包增量兜底只能把结果**向上**翻成中奖：钻石无增量不等于未中奖（非钻石奖品增量为 0），所以「receive 一次都没到达抖音 + 钱包无正增量」只能记为开奖异常，不得伪造未中奖。
