...
红包参与设置包含持久化的“参与倒计时”门槛，默认 10 秒。Go 调度器只给仍有至少该秒数有效期的红包分配参与任务；设置为 0 时取消额外倒计时门槛，但已过期红包仍不得参与。红包库当前列表允许保留已过期不超过 30 秒的红包，展示保留不改变原生参与的实际过期校验。
...
开奖结果查询设置使用“开奖后查询延迟”（默认 1 秒）和“最多查询次数”（默认 3 次）；查询次数耗尽后必须立即执行钻石增量兜底，不能再以红包有效期或一个独立的“开奖异常等待”时长拖延参与记录。旧版开奖等待字段只用于兼容迁移，不再出现在设置界面。
